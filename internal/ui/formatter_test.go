	...
	tests := []struct {
	...
	}{
		{
			...
			result: &models.SyncResult{
				...
				DumpDuration:     80 * time.Second,
				RestoreDuration:  40 * time.Second,
				DumpSize:         1024 * 1024,
				DumpSizeOnDisk:   1024 * 1024,
				LogicalSize:      4 * 1024 * 1024,
				IndexSize:        2 * 1024 * 1024,
				CompressionRatio: 0.25,
				TablesCount:      15,
				TransportMode:    models.TransportModeDirect,
				Traffic:          models.TrafficMetrics{Mode: models.TransportModeDirect, BytesIn: 1024 * 1024, BytesOut: 512 * 1024},
				StartTime:        time.Now().Add(-2 * time.Minute),
				EndTime:          time.Now(),
			},
			expectedText: []string{"Success", "test_db", "2m 0s", "1.0 MB", "4.0 MB", "2.0 MB", "15", "Source data estimate", "Source index estimate", "Compressed dump on disk", "Downloaded from remote", "Uploaded control traffic", "Total network I/O", "DIRECT", "Compression ratio", "Average dump speed", "Dump phase", "Restore phase", "storage context, not a network metric"},
		},
		{
			name: "failed sync",
			result: &models.SyncResult{
