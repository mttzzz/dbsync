...
- 🖥️ **Полноценный TUI**: выбор баз, таблиц, настроек, подтверждение и отчёт в одном интерфейсе
- 📋 **Очередь синхронизации**: можно подготовить несколько баз за один запуск
- 🧩 **Синхронизация по таблицам**: с авто-добавлением зависимостей по внешним ключам
- 💾 **Сохранение настроек**: конфигурация сохраняется в `$HOME/.dbsync.env`
...
DBSYNC_DUMP_NETWORK_COMPRESS=true
DBSYNC_DUMP_NETWORK_ZSTD_LEVEL=7
```
...
По умолчанию приложение ищет конфигурацию в `$HOME/.dbsync.env`.
...
Основной рабочий сценарий теперь проходит через TUI: выбор баз, таблиц, параметров дампа и запуск синхронизации выполняются внутри интерфейса.
